...
public class ConfigManager implements Util
{
    ...
    public void loadConfig(final String name) {
        final List<File> files = Arrays.stream((File[])Objects.requireNonNull new File("Carbon").listFiles())).filter(File::isDirectory).collect((Collector<? super File, ?, List<File>>)Collectors.toList());
